...
/* ---- lightbox overlay ---- */
.lay-photo-wrap{position:relative;overflow:hidden;cursor:pointer;}
.lay-photo-wrap .lay-zoom-overlay{
  position:absolute;inset:0;display:flex;align-items:center;justify-content:center;
  background:rgba(0,0,0,0);transition:background .25s ease;
  pointer-events:none;
}
.lay-photo-wrap:hover .lay-zoom-overlay{background:rgba(0,0,0,.35);}
.lay-photo-wrap .lay-zoom-overlay span{
  display:flex;align-items:center;justify-content:center;
  width:48px;height:48px;border-radius:50%;background:rgba(255,255,255,.15);
  backdrop-filter:blur(4px);border:1.5px solid rgba(255,255,255,.5);
  color:#fff;font-size:24px;opacity:0;transform:scale(.8);
  transition:opacity .25s ease,transform .25s ease;
}
.lay-photo-wrap:hover .lay-zoom-overlay span{opacity:1;transform:scale(1);}
/* ---- lightbox modal ---- */
#lay-lightbox{
...
#lay-lightbox.active{display:flex;}
@keyframes layLbIn{from{opacity:0}to{opacity:1}}
#lay-lightbox-img{
  max-width:min(94vw,1100px);max-height:88vh;
  object-fit:contain;border-radius:12px;
  box-shadow:0 24px 80px rgba(0,0,0,.6);
  display:block;
  animation:layLbImgIn .25s ease;
}
@keyframes layLbImgIn{from{transform:scale(.92);opacity:0}to{transform:scale(1);opacity:1}}
#lay-lightbox-close{
...
#lay-lightbox-close:hover{background:rgba(255,255,255,.3);transform:rotate(90deg);}
...
@keyframes layLbSpin{to{transform:rotate(360deg)}}
/* ---- skeleton card ---- */
.lay-card-skeleton{
...
@keyframes laySkPulse{0%,100%{opacity:1}50%{opacity:.5}}
.lay-sk-img{height:224px;background:#e2e8f0;}
.lay-sk-body{padding:1.5rem;}
.lay-sk-line{height:.9rem;border-radius:4px;background:#e2e8f0;margin-bottom:.6rem;}
/* ---- pagination bar ---- */
#lay-pagbar{
  margin-top:3.5rem;
...
.lay-pag-info{font-size:1rem;color:#64748b;font-weight:500;}
.lay-pag-btns{display:flex;align-items:center;gap:.5rem;}
.lay-pag-btn{
  display:inline-flex;align-items:center;justify-content:center;
  height:36px;min-width:36px;padding:0 .65rem;
  border-radius:.5rem;border:1px solid #e2e8f0;
  background:#fff;color:#334155;font-size:.85rem;font-weight:500;
  cursor:pointer;transition:all .18s ease;
  user-select:none;
}
.lay-pag-btn:hover:not(:disabled){background:#f1f5f9;border-color:#94a3b8;}
.lay-pag-btn.active{background:var(--color-primary,#0a3d2f);color:#fff;border-color:var(--color-primary,#0a3d2f);}
.lay-pag-btn:disabled{opacity:.4;cursor:not-allowed;}
.lay-perpage-wrap{display:flex;align-items:center;gap:.5rem;font-size:1rem;color:#475569;font-weight:500;}
.lay-perpage-select{
  border:1px solid #e2e8f0;border-radius:.4rem;
  padding:.3rem .6rem;font-size:1rem;
  background:#fff;color:#334155;cursor:pointer;font-weight:500;
}
/* state error/empty */
#lay-state{
...
(function () {
    ...
    function haystack(item) {
        return [item.kategori, item.badge, item.nama, item.subjudul,
                (item.baris || []).map(function(b){ return b.teks; }).join(' ')
               ].join(' ').toLowerCase();
    }
    ...
    function renderPagBtns(pages) {
        ...
        if (prev) prev.addEventListener('click', function() { if (curPage > 1) { curPage--; render(); scrollToGrid(); } });
        if (next) next.addEventListener('click', function() { if (curPage < pages) { curPage++; render(); scrollToGrid(); } });
    ...
    function openLightbox(src) {
        ...
        var tmp = new Image();
        tmp.onload = function() { lbImg.src = src; lbImg.style.display = 'block'; lbSpinner.style.display = 'none'; };
        tmp.onerror = function() { lbSpinner.style.display = 'none'; };
        tmp.src = src;
    }
