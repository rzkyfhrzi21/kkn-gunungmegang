...
<style>
.material-symbols-outlined{font-size:18px;line-height:1;vertical-align:-3px}
.app-ikon-preview .material-symbols-outlined{font-size:20px}
...
.choices__item--choice.is-highlighted .app-ch-row .material-symbols-outlined { color: #fff; }
.app-ch-code {
    font-size: .7rem;
    color: #94a3b8;
...
.choices__item--choice.is-highlighted .app-ch-code { color: rgba(255,255,255,.6); }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var IKON_IDS   = [['sel-ikon0','preview-ikon0'], ['sel-ikon1','preview-ikon1']];
    ...
    function buildChoicesOptions() {
        return Object.keys(LAY_IKON_MAP).map(function (code) {
            return { value: code, label: LAY_IKON_MAP[code] };
        });
    }
        ...
        function wrapActs() {
            tbody.querySelectorAll('td.row-actions').forEach(function (cell) {
                ...
                btns.forEach(function (btn, i) {
                    ...
                    if      (act === 'edit')   { btn.innerHTML = '<i class="bi bi-pencil-square text-primary"></i>Ubah'; }
                    else if (act === 'delete') { btn.innerHTML = '<i class="bi bi-trash"></i>Hapus'; btn.classList.add('text-danger'); }
                    else if (act === 'detail') { btn.innerHTML = '<i class="bi bi-eye text-info"></i>Lihat'; }
